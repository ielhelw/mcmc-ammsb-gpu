...
struct random_init_with_seed {
  random_seed_t base_seed;

  random_init_with_seed(random_seed_t base_seed) : base_seed(base_seed) {}

  __device__
  random_seed_t operator()(uint64_t inc, random_seed_t seed) {
    random_seed_t ret;
    ret.x = base_seed.x + inc;
    ret.y = base_seed.y + inc;
    return ret;
  }
};
...
__host__
void
random_init(random_seed_t base_seed,
            thrust::device_vector<random_seed_t>* seeds) {
  thrust::device_vector<uint64_t> seq(seeds->size());
  thrust::sequence(seq.begin(), seq.end());
  thrust::transform(seq.begin(), seq.end(),
      seeds->begin(), seeds->begin(),
      random_init_with_seed(base_seed));
}
...
inline ulong xorshift_128plus(ulong2 *s) {
	...
	(*s).x = s0;
	s1 ^= s1 << 23;
	ulong res = ((*s).y = (s1 ^ s0 ^ (s1 >> 17) ^ (s0 >> 26))) + s0;
    return res;
}

__device__
inline ulong rand(random_seed_t *s) {
	ulong r = xorshift_128plus(s);
	return r;
}

__device__
inline double random(random_seed_t *s) {
	return (1.0 * rand(s) / ULONG_MAX);
}

__device__
inline int randint(random_seed_t *s, int from, int upto) {
