...
TEST_P(WgNormalizeParameterizedTest, VaryLength) {
  ...
  compute::kernel kernel = prog_.create_kernel("WG_NORMALIZE_float");
  for (auto v : vals) {
    ...
    compute::vector<compute::float_> out(in.size() / wg + 1, context_);
    kernel.set_arg(0, in);
...
TEST_F(WgNormalizeTest, CustomNormalizePerformance) {
  ...
  compute::vector<compute::float_> out(in.size(), context_);
  uint32_t wg = 32;
